...
if [ -n "$REMAINING" ]; then
  # Try auto-resolve with pi
  PI_BIN="$ROOT_DIR/fork/dist/pi-darwin-arm64/bin/pi"
  if [ -x "$PI_BIN" ]; then
    echo -e "${YELLOW}Attempting auto-resolve with pi...${RESET}"
    echo ""

    RESOLVE_PROMPT="Resolve all merge conflicts in this repository. Rules:
1. For packages/coding-agent/src/** files: prefer upstream changes, but keep any fork-specific modifications
2. For fork/**: prefer fork side
3. Remove all conflict markers (<<<<<<< ======= >>>>>>>)
4. After resolving, stage the files with git add"

    if "$PI_BIN" -p "$RESOLVE_PROMPT" \
      --no-session \
      --no-skills \
      --no-prompt-templates \
      --no-themes \
      ...
      if [ -n "$STILL_UNMERGED" ]; then
        echo -e "${YELLOW}pi could not resolve all conflicts.${RESET}"
        print_conflict_help "$STILL_UNMERGED"
        exit 1
      fi
      echo -e "${GREEN}pi resolved all conflicts.${RESET}"
      REMAINING=""
    else
      echo -e "${YELLOW}pi failed to run. Falling back to manual resolution.${RESET}"
